...
use pallet_mundis_common::{log, UpwardMessagesInterface, PayloadType};
...
pub(crate) const LOG_TARGET: &'static str = "runtime::mundis-upward-messages";
...
impl<T: Config> UpwardMessagesInterface<<T as frame_system::Config>::AccountId> for Pallet<T> {
    fn submit(_who: Option<T::AccountId>, payload_type: PayloadType, payload: &[u8]) -> Result<u64, DispatchError> {
        match payload_type {
