...
use pallet_mundis_common::{
    log,
    MinichainInterface, ConvertIntoL0Token, LposInterface, TokenIdAndAssetIdProvider,
    UpwardMessagesInterface, ValidatorsProvider,
    BurnAssetPayload, LockNftPayload, LockPayload, AxisTokenMetadata, PayloadType,
};
use scale_info::{
...
pub(crate) const LOG_TARGET: &'static str = "runtime::mundis-minichain";
...
pub const KEY_TYPE: KeyTypeId = KeyTypeId(*b"mdis");
...
pub mod sr25519 {
    ...
    sp_application_crypto::with_pair! {
		/// An L1 keypair using sr25519 as its crypto.
		pub type AuthorityPair = app_sr25519::Pair;
	}

    /// An L1 signature using sr25519 as its crypto.
    pub type AuthoritySignature = app_sr25519::Signature;

    /// An L1 identifier using sr25519 as its crypto.
...
pub mod ecdsa {
    ...
    sp_application_crypto::with_pair! {
		/// An L1 keypair using ecdsa as its crypto.
		pub type AuthorityPair = app_ecdsa::Pair;
	}

    /// An L1 signature using ecdsa as its crypto.
    pub type AuthoritySignature = app_ecdsa::Signature;

    /// An L1 identifier using ecdsa as its crypto.
    pub type AuthorityId = app_ecdsa::Public;
}

type BalanceOf<T> = <<T as Config>::Currency as Currency<<T as frame_system::Config>::AccountId>>::Balance;

/// Validator of minichain.
...
pub struct Validator<AccountId> {
    ...
    validator_id_in_minichain: AccountId,
...
/// Minichain token burn event.
#[derive(Deserialize, Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo)]
pub struct BurnEvent<AccountId> {
    #[serde(default)]
    index: u32,
    #[serde(rename = "sender_id_in_layer0")]
    #[serde(with = "serde_bytes")]
    sender_id: Vec<u8>,
    #[serde(rename = "receiver_id_in_minichain")]
    #[serde(deserialize_with = "deserialize_from_hex_str")]
    #[serde(bound(deserialize = "AccountId: Decode"))]
    receiver: AccountId,
    #[serde(deserialize_with = "deserialize_from_str")]
...
#[derive(Deserialize, Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug, TypeInfo)]
pub enum MinichainNotification<AccountId> {
    #[serde(rename = "Layer0FungibleTokenLocked")]
    #[serde(bound(deserialize = "AccountId: Decode"))]
    LockAsset(LockAssetEvent<AccountId>),

    #[serde(rename = "WrappedMinichainTokenBurnt")]
    #[serde(bound(deserialize = "AccountId: Decode"))]
    Burn(BurnEvent<AccountId>),

    // #[serde(rename = "WrappedNonFungibleTokenBurnt")]
    #[serde(rename = "WrappedMinichainNFTLocked")]
    #[serde(bound(deserialize = "AccountId: Decode"))]
...
impl<T: Config> MinichainInterface for Pallet<T> {
...
pub mod pallet {
    ...
    pub trait Config: CreateSignedTransaction<Call<Self>> + frame_system::Config {
        /// Identity of an minichain authority.
        ...
        type CollectionId: Member + Parameter + Default + Copy + HasCompact + From<u128> + Into<u128>;

        /// The type used to identify a unique nft asset within an asset class.
        type ItemId: Member + Parameter + Default + Copy + HasCompact + From<u128> + Into<u128>;
        ...
        type Convertor: ConvertIntoL0Token<CollectionId=Self::CollectionId, ItemId=Self::ItemId>;
    ...
    /// A map from Layer0 token account ID to minichain asset ID.
    #[pallet::storage]
    ...
    /// Whether the minichain is activated.
    ///
    /// Only an active minichain will communicate with the mainchain and pay block rewards.
    #[pallet::storage]
    ...
    #[pallet::storage]
    #[pallet::getter(fn mundis_pallet_id)]
    pub type MundisPalletId<T: Config> = StorageValue<_, Option<T::AccountId>, ValueQuery>;
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            ...
            <MundisPalletId<T>>::put(Some(account_id));
    ...
    pub enum Event<T: Config> {
        ...
        /// An `amount` of native token has been locked in the minichain to indicate that
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn offchain_worker(block_number: T::BlockNumber) {
            ...
            // Only communicate with L0 if we are validators.
            match Self::get_validator_id() {
                Some((public, key_data, validator_id)) => {
                    ...
                    let l0_rpc_endpoint = Self::get_mainchain_rpc_endpoint(
                        anchor_contract[anchor_contract.len() - 1] == b't',
                    );
                    // last byte is 't'
                    log!(debug, "current l0_rpc_endpoint {:?}", l0_rpc_endpoint);
        ...
        fn on_runtime_upgrade() -> Weight {
            ...
            log!(
				info,
				"Running migration in L1 minichain pallet with current storage version {:?} / onchain {:?}",
				current,
				onchain
			);
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_observations(
            ...
            for observation in payload.observations.iter() {
                if let Err(e) = Self::submit_observation(&val_id, observation.clone()) {
                    log!(warn, "L1-ALERT submit_observation: Error: {:?}", e);
                }
            }
        ...
        // 1. native token on minichain
        // mainchain:mint() <- minichain:lock()
        // mainchain:burn() -> minichain:unlock()
        //
        // 2. token on mainchain
        // mainchain:lock_asset()   -> minichain:mint_asset()
        // mainchain:unlock_asset() <- minichain:burn_asset()
        ...
        // mainchain:mint() <- minichain:lock_nft()
        // mainchain:burn() -> minichain:unlock_nft()
        ...
        pub fn lock_nft(
            ...
            let metadata = match T::Convertor::convert_into_l0_token_metadata(class, instance) {
                Some(data) => data,
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn get_mainchain_rpc_endpoint(is_testnet: bool) -> String {
            ...
            if let Some(data) = sp_io::offchain::local_storage_get(
                kind,
                b"mundis_minichain::l0_rpc_endpoint",
            ) {
        ...
        fn should_send(block_number: T::BlockNumber) -> bool {
            ...
            let val = StorageValueRef::persistent(b"mundis_minichain::last_send");
        ...
        pub(crate) fn observing_mainchain(
            ...
            if obs.len() == 0 {
                log!(debug, "No validat_set updates, try to get minichain notifications.");
                // Make an external HTTP request to fetch the current price.
                // Note this call will block until response is received.
                let ret = Self::get_minichain_notification_histories(
                    mainchain_rpc_endpoint,
                    anchor_contract.clone(),
                ...
                match ret {
                    ...
                    Err(_) => {
                        return Err("Failed to get_minichain_notification_histories");
                    }
                }
            }
            ...
            if result[0].1.is_err() {
                log!(warn, "L1-ALERT Failed to submit observations: {:?}", result[0].1);
                ...
                match observation.clone() {
                    Observation::UpdateValidatorSet(val_set) => {
                        let validators: Vec<(T::AccountId, u128)> = val_set
                            .validators
                            .iter()
                            .map(|v| (v.validator_id_in_minichain.clone(), v.total_stake))
                            .collect();
        ...
        fn validate_transaction_parameters(
            ...
            ValidTransaction::with_tag_prefix("MundisMinichain")
        ...
        fn migration_to_v1() {
            ...
            MundisPalletId::<T>::put(Some(account));
