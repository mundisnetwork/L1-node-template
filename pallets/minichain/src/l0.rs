...
#[derive(Deserialize, RuntimeDebug)]
pub struct MinichainNotificationHistory<AccountId> {
    #[serde(bound(deserialize = "AccountId: Decode"))]
    minichain_notification: MinichainNotification<AccountId>,
...
impl<T: Config> Pallet<T> {
    ...
    pub(super) fn get_minichain_notification_histories(
        rpc_endpoint: &str,
        anchor_contract: Vec<u8>,
        index: u32,
        limit: u32,
        ...
        let args = Self::encode_get_notification_args(index, limit).ok_or_else(|| {
            log!(info, "Encode get_minichain_notification_histories args error");
            http::Error::Unknown
        })?;
        ...
        body.extend(
            br#"",
				"method_name": "get_minichain_notification_histories",
				"args_base64": ""#,
        );
        body.extend(&args);
        body.extend(
        ...
        let notifications: Vec<
            MinichainNotificationHistory<<T as frame_system::Config>::AccountId>,
        > = serde_json::from_slice(&json_response.result.result).map_err(|_| {
            log!(warn, "Failed to decode minichain notification histories");
            http::Error::Unknown
        })?;
        log!(debug, "minichain notifications: {:#?}", notifications);

        for n in notifications.iter() {
            match n.minichain_notification.clone() {
                MinichainNotification::Burn(mut event) => {
                    event.index = n.index;
                    obs.push(Observation::Burn(event));
                },
                MinichainNotification::LockAsset(mut event) => {
                    event.index = n.index;
                    obs.push(Observation::LockAsset(event));
                },
                MinichainNotification::BurnNft(mut event) => {
                    event.index = n.index;
