...
use pallet_mundis_common::{
    log, MinichainInterface, LposInterface, UpwardMessagesInterface, ValidatorsProvider, EraPayoutPayload, Offender, PayloadType, PlanNewEraPayload,
};
...
pub(crate) const LOG_TARGET: &'static str = "runtime::mundis-pos";
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config + SendTransactionTypes<Call<Self>> {
        ...
        type MinichainInterface: MinichainInterface;
    ...
    pub type ErasStakers<T: Config> = StorageDoubleMap<_, Twox64Concat, EraIndex, Twox64Concat, T::AccountId, u128, ValueQuery>;
    ...
    pub type ErasRewardPoints<T: Config> = StorageMap<_, Twox64Concat, EraIndex, EraRewardPoints<T::AccountId>, ValueQuery>;
    ...
    /// Offenders that to be reported to L0.
    #[pallet::storage]
    #[pallet::getter(fn offenders)]
    pub type Offenders<T: Config> = StorageDoubleMap<_, Twox64Concat, Kind, Twox64Concat, T::AccountId, u32, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_history_depth(origin: OriginFor<T>, #[pallet::compact] new_history_depth: EraIndex, #[pallet::compact] _era_items_deleted: u32) -> DispatchResult {
            ensure_root(origin)?;
...
impl<T: Config> Pallet<T> {
    ...
    fn new_session(session_index: SessionIndex, _is_genesis: bool) -> Option<Vec<T::AccountId>> {
        if let Some(current_era) = Self::current_era() {
            ...
            if era_length < T::SessionsPerEra::get() {
                ...
                if T::MinichainInterface::is_activated() &&
                    (era_length == T::SessionsPerEra::get() - 1)
                {
                    let next_set_id = T::MinichainInterface::next_set_id();
    ...
    fn end_era(active_era: ActiveEraInfo, _session_index: SessionIndex) {
        if !T::MinichainInterface::is_activated() || <EraPayout<T>>::get() == 0 {
            return;
        }
